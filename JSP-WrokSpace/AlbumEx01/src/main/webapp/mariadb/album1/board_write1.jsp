	...
	window.onload = function() {
		document.getElementById( 'wbtn' ).onclick = function() {
			...
			if( document.wfrm.upload.value.trim() == '' ) {
				...
				// 파일명에서 확장자 분리해서 출력
				const ext = document.wfrm.upload.value.split('.');
				// alert( ext[ext.length -1]);
				
				if(ext[ext.length -1].toLowerCase() != 'jpg' && ext[ext.length -1].toLowerCase() != 'png' && ext[ext.length -1].toLowerCase() != 'gif' &&  ext[ext.length -1].toLowerCase() != 'jpeg') {
